...
export function addTask(content, text, section = 'current') {
  ...
  // Use function replacement to avoid $ special chars in task text being interpreted
...
export function moveTask(content, text, fromSection, toSection) {
  ...
  for (const line of lines) {
    const h = line.match(/^## (Current|Backlog|Done)/i);
    if (h) {
      currentSection = h[1].toLowerCase();
      newLines.push(line);
      // Insert immediately after the destination section header
    ...
    // Remove the matching task from the source section (first match only)
    if (currentSection === fromSection && !removed) {
      const m = line.match(/^- \[[ x]\] (.+)/);
      if (m && m[1].replace(/ — \d{4}-\d{2}-\d{2}$/, '') === text) {
        removed = true;
        continue;
      }
    }
  ...
  if (!inserted) {
    // destination section header missing — append it
    const hdr = toSection === 'backlog' ? '## Backlog' : '## Current';
