...
export function launchTui() {
  ...
  // ── pick from list ───────────────────────────────────────────────────────────
  function pickTask(proj, section, label, cb) {
    const content = readFile(todoPath(proj.dir));
    if (!content) { updateStatus('No task file found'); return; }
    const tasks = parseTasks(content)[section];
    if (!tasks.length) { updateStatus(`No ${section} tasks`); return; }

    const box = blessed.list({
      top: 'center', left: 'center', width: 62,
      height: Math.min(tasks.length + 4, 16),
      border: { type: 'line' },
      label: ` {bold}{#6366f1-fg} ${label} — ${proj.name} {/} `,
  ...
  screen.key('d', () => {
    pickTask(currentProj(), 'current', 'Mark done', text => {
      const proj = currentProj();
      const p = todoPath(proj.dir);
  ...
  screen.key('m', () => {
    pickTask(currentProj(), 'backlog', 'Promote → current', text => {
      const proj = currentProj();
      const p = todoPath(proj.dir);
  ...
  screen.key('D', () => {
    pickTask(currentProj(), 'current', 'Demote → backlog', text => {
      const proj = currentProj();
      const p = todoPath(proj.dir);
