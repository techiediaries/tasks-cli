...
import { parseTasks, parseNotes, addTask, markDone, addToDone, addNote, initTodo, initNotes } from './md.js';
import { findProjects, todoPath, notesPath, readFile, writeFile } from './discover.js';
...
export function launchTui() {
  ...
  function updateHeader() {
    ...
    header.setContent(`  ✦ Tasks${pinLabel}   [a]add [b]backlog [d]done [n]note [p]pin [q]quit`);
    screen.render();
  }
  ...
  function updateStatus(msg) {
    const base = `  {bold}{#6366f1-fg}[a]{/} add  {bold}{#6366f1-fg}[b]{/} backlog  {bold}{#6366f1-fg}[d]{/} done  {bold}{#6366f1-fg}[n]{/} note  {bold}{#6366f1-fg}[p]{/} pin  {bold}{#6366f1-fg}[q]{/} quit`;
