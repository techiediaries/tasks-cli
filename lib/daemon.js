...
function findTerminal(nodeExec, tasksBin, captureArg) {
  const candidates = [
    ...
    ['gnome-terminal', '--geometry=66x10', '--title=Tasks: Quick Capture',
      '--', nodeExec, tasksBin, captureArg],
    // xfce4-terminal
    ['xfce4-terminal', '--geometry=66x10', '--title=Tasks: Quick Capture',
      '-x', nodeExec, tasksBin, captureArg],
  ];
...
function writeHotkeyConfig(hotkey, nodeExec, tasksBin, captureArg) {
  const termArgs = findTerminal(nodeExec, tasksBin, captureArg);
  if (!termArgs) {
    throw new Error(
  ...
  // Use single quotes for args with spaces — double quotes would break the
  // outer "..." that xbindkeys wraps the whole command in.
...
export function daemonMain({ hotkey = 'control+space' } = {}) {
  ...
  const captureArg = 'capture';

  try {
    writeHotkeyConfig(hotkey, nodeExec, TASKS_BIN, captureArg);
  } catch (err) {
